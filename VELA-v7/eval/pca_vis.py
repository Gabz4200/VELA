...
def load_model_and_processor(
    ckpt_path: str,
    config_name: str,
    hub_repo: str = "tiiuae/siglino-0.6B",
    device: str = "cuda",
    max_pixels: int = 256 * 256,
    quantize: bool = False,
):
    """Load the standalone Falcon Vision model and image processor."""
    # Ensure dtype logic matches VELA requirements (FP16 on CUDA, FP32 on CPU)
    dtype = torch.bfloat16 if "cuda" in device else torch.float32

    if ckpt_path:
        print(f"Loading local checkpoint: {ckpt_path}  config={config_name}")
        ...
        print(f"Loading from HF Hub: {hub_repo}")
        model, processor = load_siglino_from_hub(
            repo_id=hub_repo,
            device=device,
            dtype=dtype,
            quantize=quantize,
