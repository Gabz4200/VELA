...
parser.add_argument(
    "--choices", default="{tier}_choices.json", type=str, help="Choices file name format."
)
parser.add_argument(
...
def wavg(lst, w):
    if sum(w) == 0:
        return None
    return float(sum(lst[i] * w[i] for i in range(len(lst)))) / sum(w)
...
def intsFromSlice(strSlice):
    if ":" not in strSlice:
        return range(int(strSlice))
    sliceLims = [int(n) for n in strSlice.split(":")]
    return range(*sliceLims)
...
def computeGroundingScore(question, sceneGraph, attentionMap):
    ...
    regions += [
        getRegion(sceneGraph, pointer) for pointer in question["annotations"]["question"].values()
    ]
    # add answer regions
    regions += [
        getRegion(sceneGraph, pointer) for pointer in question["annotations"]["fullAnswer"].values()
    ]
