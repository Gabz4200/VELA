...
def main():
    ...
    parser.add_argument(
        "--max-workers", type=int, default=1, help="Maximum number of parallel workers (default: 1)"
    )
    parser.add_argument(
        "--devices",
        type=str,
    ...
    tasks = [
        (pdf_path, output_dir, devices[i % num_devices]) for i, pdf_path in enumerate(pdf_files)
    ]
    ...
    with Pool(processes=num_workers) as pool:
        for _ in tqdm(
            pool.imap_unordered(process_pdf, tasks), total=len(tasks), desc="Processing PDFs"
        ):
            pass
