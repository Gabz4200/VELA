...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--grid_size", type=int
    )  # -1 for no grid, 0 for cls token, 1 for global avg, 8 for 64 tokens
    ...
    parser.add_argument(
        "--hyperparameter_values", type=str, help="comma separated list of hyperparameter values"
    )
    ...
    for task in task_names:
        for value in hyperparameter_values:
            print(
                f"Running task: {task} with hyperparameter: {args.hyperparameter} and value: {value}"
            )
            ...
            print(
                f"Completed task: {task} with hyperparameter: {args.hyperparameter} and value: {value}"
            )
