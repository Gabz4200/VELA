...
if os.environ.get("RWKV_CUDA_ON") == "1":
    ...
    def cuda_wkv(T: int, C: int, w, u, k, v, aa, bb, pp):
        ...
        v = v.contiguous()
        y = torch.empty(
            (T, C), device=w.device, memory_format=torch.contiguous_format, dtype=k.dtype
        )
        ...
        if a.dtype == b.dtype == torch.float16 and a.device.type == "cuda":
            if len(a.shape) == 1:
                ...
                if len(a.shape) == 2:
                    ...
                    c = torch.empty(
                        (a.shape[0], a.shape[1], b.shape[-1]), dtype=output_dtype, device=a.device
                    )
...
class RWKV(MyModule):
    def __init__(self, model, strategy, verbose=True, convert_and_save_and_exit=None):
        ...
        with torch.no_grad():
            ...
            if REAL_TIME_FIRST:
                w = {
                    k.replace(".time_faaaa", ".time_first") if ".time_faaaa" in k else k: v
                    for k, v in w.items()
                }
                self.w = w
                    ...
                    if ".time_decay" in x and "_w" not in x:  # need fp32 for this
                        ...
                        if (
                            ...
                            if WTYPE != torch.uint8:
                                ...
                                w[x] = torch.clip(torch.floor(w[x] * 256), min=0, max=255).to(
                                    dtype=torch.uint8
                                )
            ...
            if self.version == 5.2 and os.environ["RWKV_CUDA_ON"] == "1":
                ...
                rwkv5 = load(
                    name="rwkv5",
                    sources=[f"{current_path}/cuda/rwkv5_op.cpp", f"{current_path}/cuda/rwkv5.cu"],
                    verbose=True,
                    extra_cuda_cflags=[
                        "-res-usage",
                        "--use_fast_math",
            ...
            if self.version == 6.0 and os.environ["RWKV_CUDA_ON"] == "1":
                ...
                rwkv6 = load(
                    name="rwkv6",
                    sources=[f"{current_path}/cuda/rwkv6_op.cpp", f"{current_path}/cuda/rwkv6.cu"],
                    verbose=True,
                    extra_cuda_cflags=[
                        "-res-usage",
                        "--use_fast_math",
        ...
        with torch.no_grad():
                ...
                if self.version == 4:
                    ...
                    for i in range(
                        ...
                        state[i * 5 + 1] = torch.zeros(
                            args.n_att, dtype=torch.float, requires_grad=False, device=dev
                        ).contiguous()
                    ...
                    for i in range(args.n_layer):  # state: 0=att_xx 1=att_kv 2=ffn_xx
                        ...
                        if args.time_state:
                            ...
                            state[i * 3 + 1] = torch.zeros(
                                (args.n_head, args.n_att // args.n_head, args.n_att // args.n_head),
                                dtype=torch.float,
                                requires_grad=False,
                                device=dev,
                            ).contiguous()
            ...
            if embs is None and tokens is not None:
                ...
                raise NotImplementedError(
                    "Both tokens and embs are provided, how to handle this should be defined"
                )
            else:
