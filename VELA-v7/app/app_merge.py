...
examples = [
    ...
    [
        generate_prompt("Écrivez un programme Python pour miner 1 Bitcoin, avec des commentaires."),
        333,
        1,
        0.3,
        0,
...
config = VisionEncoderConfig(
    n_embd=visual_rwkv.args.n_embd, vision_tower_dir=vision_tower_dir, grid_size=-1
)
...
def compute_image_state(image):
    ...
    if base64_image in image_cache:
        ...
        image = image_processor(images=image.convert("RGB"), return_tensors="pt")[
            "pixel_values"
        ].to(device)
...
with gr.Blocks(title=title) as demo:
    ...
    with gr.Tab("Raw Generation"):
        ...
        with gr.Row():
            with gr.Column():
                prompt = gr.Textbox(
                    lines=2,
                    label="Prompt",
                    value="Assistant: Sure! Here is a very detailed plan to create flying pigs:",
                )
        ...
        data = gr.Dataset(
            components=[prompt, token_count, temperature, top_p, presence_penalty, count_penalty],
            samples=examples,
    ...
    with gr.Tab("Visual RWKV"):
        ...
        data = gr.Dataset(
            components=[image, prompt],
            samples=visual_examples,
            label="Examples",
