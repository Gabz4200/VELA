...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_model", default="", type=str, help="path of rwkv model"
    )  # full path, with .pth
    ...
    parser.add_argument(
        "--n_vtc_layer", default=3, type=int
    )  # number of layers in the visual token compressor
    ...
    try:
        ...
        pass
    pl_version = getattr(pl, "__version__", "unknown")
    rank_zero_info(
        f"""
############################################################################
#
...
# Found pytorch_lightning {pl_version}, recommend 1.9.5
#
############################################################################
"""
    )
    ...
    trainer = Trainer.from_argparse_args(args, callbacks=[train_callback(args)])  # type: ignore
