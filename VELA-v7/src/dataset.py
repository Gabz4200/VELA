...
from .utils import image_to_regions, largest_3n_plus_2_prime
...
def process_image_tokens_in_conversations(
    conversations: Sequence[Dict], num_regions: int
) -> Sequence[Dict]:
    """
...
def _add_speaker_and_signal(conversations):
    ...
    for sentence in conversations:
        ...
        if from_str in ["human", "user"]:
            role = "user"
        elif from_str in ["gpt", "assistant"]:
            role = "assistant"
        ...
        if content is not None and content != "":
            sentence["value"] = f"{header}{content}<im_end>\n"
        else:
            sentence["value"] = header

    return conversations


def tokenize_with_image_token(
    prompt, tokenizer, num_token_per_image, image_token_index=IMAGE_TOKEN_INDEX
):
...
def preprocess(
    ...
    for conversation in conversations:
        if has_image:
            conv_ids = tokenize_with_image_token(
                conversation["value"], tokenizer, num_token_per_image
            )
        else:
            conv_ids = tokenizer.encode(conversation["value"])

        from_str = conversation["from"].lower()
        if from_str in ["gpt", "assistant"]:
            role = "assistant"
        elif from_str in ["human", "user"]:
            role = "user"
        elif from_str == "system":
            role = "system"
        else:
    ...
    def __getitem__(self, idx):
        ...
        if "image" in sample:
            ...
            conversations = process_image_tokens_in_conversations(
                copy.deepcopy(sample["conversations"]), num_regions=len(regions)
            )
        else:
        ...
        # add sample_id
        data_dict["sample_id"] = (
            str(sample["sample_id"]) if "sample_id" in sample else str(sample["id"])
        )
        return data_dict
