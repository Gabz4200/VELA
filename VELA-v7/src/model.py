...
def wind_backstepping_ref_backward(w, q, k, v, z, b, dy, s_chunk, sa):
    ...
    for t in range(T - 1, -1, -1):
        ...
        stateT = (stateT - torch.matmul(k_t.unsqueeze(-1), v_t.unsqueeze(-2)) - \
                           torch.matmul(b_t.unsqueeze(-1), sa_t.unsqueeze(-2))) / w_t.unsqueeze(-1)
...
def detect_gpu_backend() -> tuple[str, str]:
    ...
    if mps is not None and mps.is_available():
        ...
        if not macos_major:
            ...
            raise RuntimeError(
                "Could not detect macOS major version via `platform.mac_ver()`."
            )
...
def _cpu_brand() -> str:
    ...
    if fallback:
        return fallback
    raise RuntimeError(
        "Could not detect a host-CPU brand string."
    )
...
try:
    ...
    print(f"Failed to load or compile C++/CUDA wind_backstepping extension: {e}. Falling back to pure PyTorch reference implementation.")
    HAS_CPP_EXT = False
